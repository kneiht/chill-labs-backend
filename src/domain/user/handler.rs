...
pub async fn create_user(
    ...
    match user_service.create_user(req.display_name, req.email, req.role).await {
        Ok(user) => UseCaseResponse::success_created(
            user.into(),
...
pub async fn get_user(
    ...
    match user_service.get_user_by_id(id).await {
        Ok(user) => UseCaseResponse::success_ok(
            user.into(),
            "User retrieved successfully"
        ),
        Err(e) => UseCaseResponse::failure_not_found(
            "User not found",
            Some(e.to_string())
        ),
    }
...
pub async fn update_user(
    ...
    match user_service.update_user(id, req.display_name, req.email, req.role, req.status).await {
        Ok(user) => UseCaseResponse::success_ok(
            user.into(),
